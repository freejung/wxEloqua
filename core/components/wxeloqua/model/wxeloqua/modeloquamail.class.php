...
require_once 'eloquamailservice.class.php';
$wxEloqua = $modx->getService('wxeloqua','wxEloqua',$modx->getOption('wxEloqua.core_path',null,$modx->getOption('core_path').'components/wxeloqua/').'model/wxeloqua/');
if (!($wxEloqua instanceof wxEloqua)) die('could not instantiate wxEloqua');
...
class modEloquaMailer extends modMail {
	...
	*/
    ...
    function __construct(modX &$modx, array $attributes= array()) {
        ...
        require_once $modx->getOption('core_path') . 'eloquamailservice.class.php';
        ...
        $this->deploySettings = new DeploymentSettings();
        $this->_getMailer();
    }
            ...
            if ($set) {
                ...
                switch ($type) {
                    ...
                    case 'bcc' :
                        break;
                    case 'reply-to' :
                        $this->email->ReplyToAddress = $email;
                        $this->email->ReplyToName = $name;
                        break;
                }
            }
        } elseif ($email === null) {
    ...
    public function send(array $attributes= array()) {
        ...
        $deploySettings = new DeploymentSettings(array('EmailId' => $emailId, 'DeploymentDate' => time()));
        $recipientIds = $wxEloqua->getContactsByFieldValues($this->recipients);
        $success = true;
